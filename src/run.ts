...
export async function run() {
	try {
		...
		process.env.COVERALLS_FLAG_NAME =
			process.env.COVERALLS_FLAG_NAME || core.getInput('flag-name');
		...
		if (core.getInput('parallel-finished') != '') {
			...
			request.post(
				{
					url: `${
						process.env.COVERALLS_ENDPOINT || 'https://coveralls.io'
					}/webhook`,
					body: payload,
					json: true,
				},
				(error: string, _response: Response, data: WebhookResult) => {
					...
					try {
						...
						throw new Error(
							'Parallel webhook error:' + err + ', ' + JSON.stringify(data)
						);
					}
				}
			);
		...
		process.env.COVERALLS_PARALLEL =
			process.env.COVERALLS_PARALLEL || core.getInput('parallel');
